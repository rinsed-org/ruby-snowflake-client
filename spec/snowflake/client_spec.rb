...
RSpec.describe Snowflake::Client do
  ...
  let(:database_name) { "ruby_snowflake_client_testing" }
  ...
  describe "#fetch" do
    ...
    context "with a large amount of data" do
      ...
      let(:limit) { 0 }
      let(:query) { "SELECT * FROM public.large_table LIMIT #{limit}" }
  ...
  describe "#fetch_with_database" do
    before do
      client.connect(
        account: ENV["SNOWFLAKE_ACCOUNT"],
        warehouse: ENV["SNOWFLAKE_WAREHOUSE"],
    ...
    let(:query) { "" }
    let(:result) { client.fetch_with_database(query, database_name) }
